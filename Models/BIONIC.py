...
try:
    ...
    gp = None  # Only required for the RBF-kernel sparse feature-selection option (SparseELBO)
...
class BIONIC(object):    
    ...
    def fit_iterate(self, max_iter = 250, pruning_crit = 1e-3, tol = 1e-3, feat_crit = 1e-3, perc = False, verbose = 0, Y_tst = [None], 
...
#        return (self.sigmoid(x) - 0.5)/(2*x)
    ...
    def update_w(self, m):
        ...
        if self.sparse[m]:
            ...
            # cho = np.linalg.cholesky(np.outer(tmp,tmp)*q.G['prodT'] + np.eye(q.Kc)/q.tau_mean(m))            
...
class SparseELBO(nn.Module):
    ...
    def __init__(self, X, V, fs, lr=1e-3, kernel='rbf', var = None):
        ...
        if kernel == 'rbf' and gp is None:
            raise ImportError(
                "The 'rbf' kernel option requires pyro-ppl, which is not installed. "
                "Install it with `pip install pyro-ppl`, or use kernel='linear' instead."
            )
...
class HyperParameters(object):
    ...
    def __init__(self, m_i):
        ...
        for m in np.arange(m_i): 
            ...
            self.mu_a.append(1e-14)
            self.mu_b.append(1e-14)
